  ...
  return (
    <section className={`urgency-wrap py-[120px] relative overflow-hidden ${cream ? 'bg-cream' : 'bg-navy text-white'}`}>
      <div className="container relative z-10">
        {eyebrow && (
          <div className={`font-mono text-[10px] tracking-[0.2em] text-coral uppercase mb-8 pb-4 flex justify-between border-b ${cream ? 'border-navy/20' : 'border-white/30'}`}>
            {eyebrow}
          </div>
        )}
        ...
        {/* Stats */}
        {stats && (
          <div className={`grid grid-cols-3 mt-16 pt-9 gap-6 border-t ${cream ? 'border-navy/20' : 'border-white/30'}`}>
            {stats.map((s) => (
              <div key={s.n}>
