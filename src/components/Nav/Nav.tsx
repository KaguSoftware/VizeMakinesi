...
export default function Nav() {
  ...
  useEffect(() => { setOpen(false); setActiveMega(null); }, [pathname]);
  ...
  return (
    <header className="sticky top-0 z-50 bg-navy border-b border-border">
      ...
      <div className="overflow-hidden font-mono text-[11px] tracking-[0.08em] uppercase py-2 bg-navy text-white/65">
        <div className="nav-ticker-track">
          {TICKER_ITEMS.map((t, i) => (
            <span key={i} className={i === 0 || i === 4 ? 'text-coral' : ''}>
              {t}
            </span>
          ))}
        </div>
      ...
      {/* Main bar */}
      <div className="container flex items-center justify-between h-[88px] relative">
        {/* Logo */}
        <Link href="/" className="flex items-baseline gap-4 font-serif text-coral">
          <span className="italic font-bold text-[26px] tracking-[-0.02em]">
            Visa<span className="text-coral">.</span>Office
          </span>
          <span className="font-mono text-[9px] tracking-[0.18em] text-white/55 uppercase font-normal not-italic">
            — Est. 2008 ——
          </span>
        </Link>
        ...
        >
          {MEGA_MENU.map((group, i) => (
            <div key={group.label} onMouseEnter={() => handleEnter(i)}>
              <button
                className={`font-sans font-medium text-[11.5px] uppercase tracking-[0.1em] px-[18px] py-3 inline-flex items-center gap-1.5 transition-colors duration-200 ${
                  activeMega === i ? 'text-coral' : 'text-white/70 hover:text-coral'
                }`}
              >
                {group.label}
                <span
                  className="text-sm opacity-50 transition-transform duration-200"
                  style={{ transform: activeMega === i ? 'rotate(180deg)' : 'none' }}
                >
                  ↓
                </span>
              </button>
          ...
          <Link
            href="/contact"
            className="ml-4 font-sans font-medium text-[11.5px] uppercase tracking-[0.1em] px-[22px] py-[14px] bg-coral border border-coral text-navy hover:bg-transparent hover:text-coral transition-all duration-200 inline-flex items-center"
          >
            Book a Consultation →
          </Link>
        </nav>
        ...
        {/* Hamburger */}
        <button
          className="lg:hidden font-sans font-medium text-[12px] uppercase tracking-[0.1em] text-white"
      ...
      {activeMega !== null && (
        ...
        >
          <div className="container">
            <div className="grid grid-cols-[220px_1fr] gap-[60px] py-14">
              {/* Side */}
              <div className="border-r border-border pr-10">
                <div className="font-mono text-[10px] tracking-[0.2em] text-coral uppercase mb-4">
                  — {String(activeMega + 1).padStart(2, '0')} / {String(MEGA_MENU.length).padStart(2, '0')}
                </div>
                <div className="font-serif italic font-bold text-[56px] leading-none tracking-[-0.025em]">
                ...
                {MEGA_MENU[activeMega].columns.map((col, ci) =>
                  col.kind === 'feature' ? (
                    <Link
                      key={ci}
                      href={col.feature!.to}
                      className="mega-feature flex flex-col justify-between p-7 bg-navy text-white min-h-[220px] border border-border"
                    >
                      ...
                      <div className="font-sans text-[11px] tracking-[0.1em] uppercase text-coral mt-4 relative z-10">
                        Read more →
                      </div>
                    </Link>
                  ) : (
                    <div key={ci}>
                      <div className="font-mono text-[10px] tracking-[0.2em] text-muted uppercase mb-[18px] pb-[14px] border-b border-border">
                        {col.title}
                      </div>
      ...
      {open && (
        <div className="fixed inset-0 bg-paper z-[100] px-7 py-8 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-center pb-6 border-b border-border">
            ...
            <button
              className="font-sans font-medium text-[12px] uppercase tracking-[0.1em] text-navy"
            ...
            {MOBILE_LINKS.map((l) => (
              <Link
                ...
                className={`font-serif text-[32px] font-semibold py-4 border-b border-border flex justify-between items-center ${
                  pathname === l.to ? 'text-coral' : 'text-navy'
                }`}
              >
                {l.label}
                <span className="text-[22px] text-muted/60">→</span>
              </Link>
            ))}
          </nav>
          <div className="mt-8 flex gap-6 font-mono text-[12px] tracking-[0.1em] uppercase pt-6 border-t border-border">
