  ...
  return (
    ...
    >
      ...
      <button
        ...
        {...attributes}
        {...listeners}
        className="mt-2 shrink-0 cursor-grab active:cursor-grabbing text-navy/20 hover:text-navy/50 transition-colors select-none"
        aria-label="Sürükle"
      >
        ⠿
      </button>
      ...
      <button
        type="button"
        onClick={onDelete}
        className="mt-2 shrink-0 font-mono text-[10px] tracking-widest uppercase text-navy/25 hover:text-red-500 transition-colors"
        aria-label="Sil"
      >
        ✕
      </button>
...
export function RepeatableList<T extends RepeatableItem>({
  ...
  return (
    <div className="flex flex-col gap-3">
      {items.length === 0 && (
        <p className="font-mono text-[11px] text-navy/30 italic">{emptyText}</p>
      )}
